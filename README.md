...
При расчетах, проект использует данные станков. 
База данных содержится в пакете machine_tools, который устанавливается при установке зависимостей. 
Но перед использованием, необходимо иннициализмровать эту базу.
...
![Окно ввода начальных условий](input_window1.png)

Кнопка "Начать расчет" выполнит расчет и представит его в окне вывода результатов

![Окно вывода результатов расчета](result_window.png)

После этого, можно выгрузить отчет о расчете, или аернуться к предыдущему шагу
Оба окна позволяют перейти в окно настройкиек расчета.

![Окно ввода настроек](settings_window.png)
