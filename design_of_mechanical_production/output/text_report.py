...
from design_of_mechanical_production.interfaces.report_generator import IReportGenerator
...
from design_of_mechanical_production.interfaces.formatters import INumberFormatter, ITableFormatter
from design_of_mechanical_production.output.formatters import NumberFormatter, TableFormatter
...
class TextReportGenerator(IReportGenerator):
    ...
    def __init__(
        self,
        number_formatter: INumberFormatter = None,
        table_formatter: ITableFormatter = None
    ):
    ...
    def generate_report(self, workshop: Workshop) -> str:
        ...
        for operation in workshop.process.operations:
            ...
            time = self.number_formatter.format(operation.time * workshop.production_volume)
        ...
        for operation in workshop.process.operations:
            ...
            report_time = self.number_formatter.format(operation.time * workshop.production_volume)  
        ...
        for operation in workshop.process.operations:
            ...
            area = self.number_formatter.format(operation.equipment.length * operation.equipment.width *
        ...
        report.append(f"S_(С.К.П.) = {work_piece_storage_percent}% ∙ {self.number_formatter.format(workshop.zones['main_zone'].area)} "
        ...
        report.append(f"S_КОНТР = {rep_control_department} ∙ {self.number_formatter.format(workshop.zones['main_zone'].area)} "
        ...
        report.append(f"S_ДОП = {self.number_formatter.format(workshop.zones['tool_storage_zone'].area)} + "
        ...
        report.append(f"L = {self.number_formatter.format(workshop.required_area)}/({workshop_span} ∙ {workshop_nam}) = "
        ...
        report.append(f"S_Ц = {workshop_span} ∙ {workshop_nam} ∙ {self.number_formatter.format(workshop.length)} = "
